...
# My answer: No, the munsters hash will be used as an argument for the mess_with_demographics method but inside that method, the each method is called on the values. The each method will call the block to change the values but then return the original hash.

# LS answer: Spot will find himself in the "dog" house for this one. The family's data is all in shambles now.
...
# Additional information on answer: The object ids of the hash values change, there is reassignment of those. The object id of the hash itself does not change, the hash is mutated by the reassignment of the values.

# The return from calling the mess_with_demographics method with the munsters hash as an argument is an array of all the reassigned nested hashes:
# => [{"age"=>74, "gender"=>"other"}, {"age"=>72, "gender"=>"other"}, {"age"=>444, "gender"=>"other"}, {"age"=>52, "gender"=>"other"}, {"age"=>65, "gender"=>"other"}]

# Within the main method we are calling the Hash#value method:
# values → array
# Returns a new array populated with the values from hsh.
# Then the each method is called on this array
# each {|item| block} → ary
# Calls the given block once for each element in self, passing that element as a parameter. Returns the array itself.
# Why isn't this leading to the original hash being returned from the mess_with_demographics method?
