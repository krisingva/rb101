...
def tfidf(term, document, documents)
  ...
  #puts "tfidf for term #{term} is #{num}"
  num
end
...
def tf(term, document)
  num = document.split(/[\s-]/).count { |word| word.downcase == term } # /[\s-]/ matches space or hyphen
  #puts "tf for term #{term} is #{num}"
  num
end
...
def idf(term, documents)
  ...
  #puts "number of documents #{number_of_documents}"
  #puts "number of documents with term #{number_of_documents_with_term}"

  num = Math.log(number_of_documents.to_f / number_of_documents_with_term.to_f)
  #puts "idf for term #{term} is #{num}"
  num
end
...
# idf value should never be 0 but it is for the term cat.
# num = Math.log(number_of_documents/ number_of_documents_with_term) would give
# 0 for an answer under 1, to get more signifcant digits to the answer, change
# the two numbers for the log operation to floats:
# num = Math.log(number_of_documents.to_f / number_of_documents_with_term.to_f)
