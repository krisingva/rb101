...
def longest_sentence(file)
  str = get_file_as_string(file)
  hsh = Hash.new
  count =  str.chars.count { |x| PUNCTUATION.include? (x) }
  # puts "The number of sentences in text is #{count.to_s}"
  index_arr = []
  str.length.times do |i|
    index_arr << i if PUNCTUATION.include? (str[i])
  end
  # puts "\n"
  # puts "The indices of punctuation within the text are:"
  # p index_arr
  start = 0
  0.upto(count-1) do |int|
    new_str = str[start..(index_arr[int])]
    words = new_str.split.size
    hsh[new_str] = words
    start = index_arr[int] + 2
  end
  # puts "\n"
  # puts "The word counts for each sentence:"
  # p hsh
  longest = hsh.values.max
  puts "\n"
  puts "The longest sentence in the text is: #{hsh.key(longest)}"
  puts "\n"
  puts "The word count in that sentence is: #{longest.to_s}"
end

#longest_sentence("1.txt")
...
# the end of one sentence and the beginning of another. Because of that fact, we
# can use these punctuation marks as places to split our string of text into
# sentences.

# On this third line, we take our array of sentences and use the
# Enumerable#max_by method to find the longest one.
...
# the sentence. Can you write a solution that keeps the period printed at the
# end of each sentence?

# What about finding the longest paragraph or longest word? How would we go
# about solving that problem?

# If you haven't used regex before, check out our Introduction to Regular
# Expressions book.
